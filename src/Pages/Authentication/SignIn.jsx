...
function SignIn() {
    ...
    async function userSignIn() {
        try {
            ...
            console.log(user);
            if(user.signInUserSession.accessToken.payload["cognito:groups"][0]=='Admins'){
  ...
  return (
    <>
         <div className="text-center mt-24">
                ...
                <span className="text-sm">or <a href="#" className="text-blue-500" onClick={()=>navigate('/signup')}> 
                    register a new account
                </a>
            </span>
            </div>
            <div className="flex justify-center my-2 mx-4 md:mx-0">
            <form className="w-full max-w-xl bg-white rounded-lg shadow-md p-6" onSubmit={signIn}>
                <div className="flex flex-wrap -mx-3 mb-6">
                    <div className="w-full md:w-full px-3 mb-6">
                        <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Email address</label>
                        <input className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none" onChange={(e)=>setEmail(e.target.value)} type='email' required/>
                    </div>
                    <div className="w-full md:w-full px-3 mb-6">
                        <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Password</label>
                        <input className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none" onChange={(e)=>setPassword(e.target.value)} type='password' required/>
                    </div>
